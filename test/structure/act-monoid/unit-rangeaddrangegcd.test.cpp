...
using Monoid = nono::act_monoid::RangeAddRangeGcd<int>;
using Value = Monoid::Value;
using Act = Monoid::Act;

bool operator==(Value lhs, Value rhs) {
    return lhs->left == rhs->left && lhs->right == rhs->right && lhs->tot == rhs->tot;
}
...
void solve() {
    ...
    const int T = 10;
    for (int _ = 0; _ < T; _++) {
        Value value[3] = {{(int)(rng() % lim)}, {(int)(rng() % lim)}, {(int)(rng() % lim)}};
        Act act[3] = {(int)(rng() % lim), (int)(rng() % lim), (int)(rng() % lim)};
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                for (int k = 0; k < 3; k++) {
